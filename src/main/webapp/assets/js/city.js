var CITY = {
	init:function()
	{
		...
		$(".btnsearcharea").click(function()
		{
			...
			$(this).prevAll().last().empty();
			CITY.areasearch(areaname);
		});
		//搜索district
	...
	findcityinfo:function( element )
	{
		...
		$.get("/city/findcitybyid/"+a,function(city){
			...
			$.get("/area/findareabycityid/"+a,function(data)
			{
				...
				areatable.empty();
				areatable.append('<tr><td>区域编号</td><td>区域名称</td><td>包含片区</td><td>区域简介</td><td>操作</td></tr>')
	...
	createarea:function( area )
	{
		...
		Area.append(areaId).append(areaName).append(district).append(remark).append(btnop);
